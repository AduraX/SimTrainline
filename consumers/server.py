...
def run_server():
    ...
    if topic_check.topic_pattern_match("Stream_Egress") is False: # Chicago_Station_Stream_Table
        logger.fatal(
            "Ensure that Faust Streaming is running successfully before running the web server!"
        )
        exit(1)

#     if topic_check.topic_exists("Chicago_Station_Stream_Egress") is False: # Chicago_Station_Stream_Table  # "Chicago_Station_Stream_egress"
#         logger.fatal(
#             "Ensure that Faust Streaming is running successfully before running the web server!"
#         )
#         exit(1)
    ...
    consumers = [
        KafkaConsumer(
            "\w*_Weather\w*", # "(\w*|\.)*weather(.(\w*|\.))*",
            weather_model.process_message,
            offset_earliest=True,
        ),
        KafkaConsumer(
            "\w*_Stream_Egress\w*", # "(\w*|\.)*stations.table(.(\w*|\.))",
            lines.process_message,
            offset_earliest=True,
            is_avro=False,
        ),
        KafkaConsumer(
             "\w*_Arrivals\w*", # "(\w*|\.)*station.arrivals.(.(\w*|\.))*",
            lines.process_message,
            offset_earliest=True,
        ),
        KafkaConsumer(
