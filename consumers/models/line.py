...
class Line:
    ...
    def process_message(self, message):
        ...
        if message.topic() == "Chicago_Station_Stream_Egress_Table": # Set the conditional correctly to the stations Faust Table
            try:
                value = json.loads(message.value())
                self._handle_station(value)
            except Exception as e:
                logger.fatal("bad station? %s, %s", value, e)
        elif "Chicago_Station_Stream_Ingest" in message.topic(): # Set the conditional to the arrival topic
